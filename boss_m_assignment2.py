...
def parse_line(line):
    #Parse a single CLF line into its fields. Expected format: host ident authuser [date] "request" status bytes
    try:
        parts = line.split()
        host = parts[0]
...
def within_range(date_obj, start_date, end_date):
    #Check if a datetime object falls within the start and end date 
